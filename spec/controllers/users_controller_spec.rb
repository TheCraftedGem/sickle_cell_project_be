...
RSpec.describe Api::V1::UsersController, type: :api do
  context 'When a user signs up' do
    it 'responds with a 200 status and sends an email' do
    ...
    it 'sends the email to the correct user' do
      ...
      expect(ActionMailer::Base.deliveries[0].body.encoded).to include("#{User.find(1).first_name}")
      expect(ActionMailer::Base.deliveries[0].body.encoded).to include("#{User.find(1).confirmation_code}")
    end
  end
end